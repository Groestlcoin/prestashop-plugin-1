<div class="panel kpi-container">
	<fieldset>
		<legend>{l s='GRSPay Information' mod='btcpay'}</legend>
		<div id="info">
			<dl class="well list-detail">
				<dt>{l s='Groestlcoin address' mod='btcpay'}</dt>
				...
				<dt>{l s='Cart value in Groestlcoin' mod='btcpay'}</dt>
				<dd>{$payment_details.btc_price} GRS</dd>
				{if $payment_details.btc_paid > 0}
					<dt>{l s='Amount paid' mod='btcpay'}</dt>
					<dd><span class="badge badge-success">{$payment_details.btc_paid} GRS</span></dd>
					{if $payment_details.btc_paid != $payment_details.btc_price}
						<dt>{l s='Payment issue' mod='btcpay'}</dt>
						<dd><span class="badge badge-warning">{l s='Amount paid not matching cart value' mod='btcpay'}</span></dd>
					{/if}
				{/if}
			</dl>
		</div>
